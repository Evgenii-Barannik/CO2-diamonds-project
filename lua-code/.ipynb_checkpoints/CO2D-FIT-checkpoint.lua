...
--Nitrogen and Platelets mode
		if mode == "n" then --Mode check
			...
			F:execute ("$spec%04d_hO = ~0 [0:]" % j) -- Create the variable for the B-center height

			F:execute ("$spec%04d_hB = ~0 [0:]" % j) -- Create the variable for the B-center height

			F:execute ("$spec%04d_hA = ~0 [0:]" % j) -- Create the variable for the A-center height
			...
			F:execute ("fit @%d" % j) --Fit
			if F:calculate_expr("$spec%04d_hB" % j) < 10 --Logic function for Platelets. Platelets height is nonzero only if B-center absorption is > 10 cm-1.
			then F:execute ("$spec%04d_hP = 0" % j) else --  To be precise 5 cm-1 is used to make sure not to include noise.
			F:execute ("$spec%04d_hP = ~0 [0:]" % j)
			end
